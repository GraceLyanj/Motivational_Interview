        ...
        for i in tqdm(range(len(lines)), desc=f"Round-{j}"):
            ...
            if os.path.exists(f"./{args.output_dir}/Sample-{i}-Round-{j}.txt"):
              with open(f"./{args.output_dir}/Sample-{i}-Round-{j}.txt") as f:
                  ...
                  if (
                      len(temp_lines) > 40
                      or "You are motivated because" in last_line
                      or "You should highlight current state and engagement, express a desire to end the current session" in last_line
                  ):
                      continue
            goal = sample["topic"]
